$(function() {
  ...
  function loadCard(el, data) {
    ...
    if (data.IN_STOCK.toUpperCase() == 'YES') {
      htmlInStock = '<h6 class="float-left"><span class="badge badge-success">In Stock</span></h6>';
    }
    ...
    if (data.Discount != 0 && data.Discount != null) {
      ...
      htmlPrice = '<span class="font-weight-light">$<s>' + data.List_Price + '</s></span>';
    }
    else{
      htmlPrice = '$' + data.List_Price + '</span>';
    }

    let htmlAttr = '<h6>'
    //'<h6 class="bg-info text-white font-weight-bold"><small>';

    if (data.SKU_ATTRIBUTE_VALUE1 != null && data.SKU_ATTRIBUTE_VALUE1 != "" && data.SKU_ATTRIBUTE_VALUE1 != undefined) {
      htmlAttr += '<span class="badge badge-info">' + data.SKU_ATTRIBUTE_VALUE1 + '</span>&nbsp;'
      //data.SKU_ATTRIBUTE_VALUE1 + '&nbsp;&nbsp;';	
    }

    if (data.SKU_ATTRIBUTE_VALUE2 != null && data.SKU_ATTRIBUTE_VALUE2 != "" && data.SKU_ATTRIBUTE_VALUE2 != undefined) {
      htmlAttr += '<span class="badge badge-info">' + data.SKU_ATTRIBUTE_VALUE2 + '</span>&nbsp;';
    }

    if (data.SKU_ATTRIBUTE_VALUE3 != null && data.SKU_ATTRIBUTE_VALUE3 != "" && data.SKU_ATTRIBUTE_VALUE3 != undefined) {
      htmlAttr += '<span class="badge badge-info">' + data.SKU_ATTRIBUTE_VALUE3 + '</span>&nbsp;';
    }

    if (data.SKU_ATTRIBUTE_VALUE4 != null && data.SKU_ATTRIBUTE_VALUE4 != "" && data.SKU_ATTRIBUTE_VALUE4 != undefined) {
      htmlAttr += '<span class="badge badge-info">' + data.SKU_ATTRIBUTE_VALUE4 + '</span>&nbsp;';
    }

    if (data.SKU_ATTRIBUTE_VALUE5 != null && data.SKU_ATTRIBUTE_VALUE5 != "" && data.SKU_ATTRIBUTE_VALUE5 != undefined) {
      htmlAttr += '<span class="badge badge-info">' + data.SKU_ATTRIBUTE_VALUE5 + '</span>&nbsp;';
    }

    if (data.SKU_ATTRIBUTE_VALUE6 != null && data.SKU_ATTRIBUTE_VALUE6 != "" && data.SKU_ATTRIBUTE_VALUE6 != undefined) {
      htmlAttr += '<span class="badge badge-info">' + data.SKU_ATTRIBUTE_VALUE6 + '</span>';
    }

    htmlAttr += '</h6>';
    //'</small></h6>';

    let htmlFav = '<a href="javascript:void(0)" >' +
                    '<i class="fas fa-heart text-muted float-right p-1 my-1 mr-3 zoom" data-toggle="tooltip" data-placement="top" title="Add to favorite"></i>' +
                  '</a>';

    //prepare the html for the card
    let cardHtml =
      '<!--Grid column-->' +
      '<div class="col-lg-3 col-md-6 mb-4 rounded-lg wow fadeIn zoom" id="p' + data.Item_Number + '">' +
      '<!--Card-->' +
      '<div class="card">' +
      '  <!--Card image-->' +
      '  <div class="view overlay">' +
      '	<img src="' + imgUrl + '" crossorigin="anonymous"  max-width="200" height="250" alt="Thumbnail" class="mx-auto d-block wow fadeIn">' +
      '	<a>' +
      ...
      '  <!--Card image-->' +
      '  <!--Card content-->' +
      '  <div class="card-body text-center cloudy-knoxville-gradient">' +
      '	<!--Category & Title-->' +
  ...
  function initPaginate(context, el) {
    ...
    let loadHTML = '<button type="button" class="btn btn-outline-primary" id="' + paginateElId + '"> &nbsp;&nbsp;Load more&nbsp;&nbsp; </button>';
